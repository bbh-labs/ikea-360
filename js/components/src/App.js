...
App.Viewer.Overlay = React.createClass({
	render: function() {
		return (
			<div id='overlay' ref='overlay' className='flex'>
				<App.Viewer.Topbar room={this.props.room} />
				<App.Viewer.Bottombar room={this.props.room} />
				<App.Viewer.Sidebar />
			</div>
		)
	},
...
App.Viewer.Bottombar = React.createClass({
	render: function() {
		return (
			<div id='bottombar' className='flex row align-start justify-end'>{
				<img className='map' src={'images/maps/' + rooms[this.props.room] + '.png'} alt='ikea logo'/>
			}</div>
		)
	},
});
